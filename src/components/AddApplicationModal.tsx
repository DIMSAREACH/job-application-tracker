...
export function AddApplicationModal({
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    if (res.success) {
      ...
      setErrorMsg(res.error || "Failed to add application");
    }
  };
      ...
      <DialogContent className="w-[95vw] sm:max-w-[680px] max-h-[90vh] overflow-y-auto rounded-3xl border-slate-200 bg-white text-slate-900 shadow-2xl dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100 p-6 sm:p-8">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Salary / Range</label>
              <Input
                {...register("salary")}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              />
              {errors.jobUrl && (
                <p className="mt-1 text-[11px] font-semibold text-rose-500">{errors.jobUrl.message}</p>
              )}
            </div>
